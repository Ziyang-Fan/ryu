...
# install all node in the path using arp ,still need to change the datapath
# finding way - 6.17
...
class ExampleSwitch13(app_manager.RyuApp):
    ...
    def get_link_cost(self, s1, s2):
        ...
        """
        e1 = self.switch_topo[s1][s2]
        e2 = self.switch_topo[s2][s1]
    ...
    def install_paths(self, src_dpid, dst_dpid, last_port, ip_src, ip_dst):
        ...
        for switch in switches_in_paths:
            ...
            # ports[i --- (match_key)] = [(out_port, weight), ..]
            ...
            for path in paths_with_ports:
                if switch in path:
                    ...
                    if (out_port, pw[i]) not in ports[i]:
                        ports[i].append(port_cost)
                i += 1
