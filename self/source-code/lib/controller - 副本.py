...
class MultipathController(app_manager.RyuApp):
    ...
    **kwargs : type
    ...
    def update_switch_path(self, start_node, end_node, first_port, last_port,
        ...
        # This is a very nicé func, so its nested in here #
        # Struct: [{switchi: (start_port, out_port), .., switchn: (out_port, end_port)}, ...]
        ...
        if arp_pkt:
            ...
            if arp_pkt.opcode == arp.ARP_REPLY:
                ...
                if end_node_ip in self.arp_table:
                    ...
                    self.update_switch_path(h2[0], h1[0], h2[1], h1[1],
                                            end_node_ip, start_node_ip, ipv6=is_ipv6
                                            )
    ...
    def _get_all_paths(self, start_node, end_node):
        ...
        #print('Av paths from {} to {} : {}'.format(start_node, end_node, paths))
        return paths
