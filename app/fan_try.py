...
class ExampleSwitch13(app_manager.RyuApp):
    ...
    def install_paths(self, src_dpid, dst_dpid, last_port, ip_src, ip_dst):
        ...
        switches_in_paths = set().union(*paths)  # [1, 3, 5, 2, 4]
        ports = defaultdict(list)
        j = 0  # match_key as counter of ports
        for switch in switches_in_paths:
            ...
            actions = []
            i = 0
            # path  {1: 3, 3: 2, 5: 3}   //   {1: 2, 2: 2, 4: 2, 5: 3}
            # {dpid: outport,  }
            for path in paths_with_ports:
                if switch in path:
                    out_port = path[switch]
                    port_cost = (out_port, pw[i])
                    if (out_port, pw[i]) not in ports[switch]:
                        ports[switch].append(port_cost)
                i += 1

            j += 1

            for out_ports in ports[switch]:
