...
class MininetApp():
    ...
    def run_benchmarks(self, iperf_iterations=3, iperf_duration=30):
        ...
        for topo_category in self.topos_to_benchmark:
            ...
            if worst_case not in self.nhop_results[n_hop] and worst_case != best_case:
            ...
            speed_result, port_data = self.benchmark_topo(
                ...
                watch_switch=self.topos_to_benchmark[topo_category]['watch_switch'],
            ...
            for nhop_test in local_nhop_tests:
                ...
                self.nhop_results[n_hop][nhop_test], ignore = self.benchmark_topo(
            ...
            speed_results[topo_category]['multipath_load_balance'] = load_balance
        # Print results
    ...
    def benchmark_topo(self, topo, cli=False, xterms=False, iperf_iterations=3,
        ...
        speed = []
        for i in range(0, iperf_iterations):
            print('Running IPerf with duration {} seconds and iteration {}/{}'.format(
                iperf_duration, i + 1, iperf_iterations))
    ...
    def get_switch_port_data(self, switch=None):
        ...
        if switch:
            try:
                ...
                # The following code pulls out the ports and the respective rxtx data
                ...
                # Returns packet data: eg. rxtx[port_number] = {'rx': 101, 'tx': 102}
                return rxtx
            except:
                return None
    ...
    def run_controller(self, force=False, silent=True):
        ...
        if not self.controller or force:
            ...
            controller = None
            if silent:
                controller = sp.Popen(['ryu-manager', '--observe-links', './lib/controller.py'],
                                      shell=False, stdout=sp.PIPE, stderr=sp.DEVNULL)
            else:
                controller = sp.Popen(
